...
class MetaEmbeddingPPI(nn.Module):
    ...
    def on_hook_forward(self, x):
        if len(x.shape) > 1:
            ...
            reshape_flag = False
        embeddings = [embed_nn(x) for embed_nn in self.embed_list]
        embeddings.append(self.ppi_nn(x))
        embed = torch.stack(embeddings, dim=-1)
        # shape of (sz_b * len_seq, n_embed, dim)
        embed = embed.permute(0, 2, 1)
        # shape of (sz_b * len_seq, n_embed, 1)
...
# Fully Connected Linear Autoencoder to reduce dimensionality of Prose PPI embeddings
class PPIAutoEncoder(nn.Module):
    def __init__(self, input_dim, embed_dim, og_embeddings):
        super(PPIAutoEncoder, self).__init__()
        self.encoder = nn.Sequential(nn.Linear(input_dim, input_dim // 2),
                                     nn.ReLU(True),
                                     nn.Linear(input_dim // 2, input_dim // 4),
                                     nn.ReLU(True),
                                     nn.Linear(input_dim // 4, embed_dim))
        self.decoder = nn.Sequential(nn.Linear(embed_dim, input_dim // 4),
                                     nn.ReLU(True),
                                     nn.Linear(input_dim // 4, input_dim // 2),
                                     nn.ReLU(True),
                                     nn.Linear(input_dim // 2, input_dim),
                                     nn.Tanh())
        self.og_embeddings = og_embeddings
        self.float()

    def forward(self, indices):
        latent_embeddings = self.encoder(self.og_embeddings[indices].float())
        return latent_embeddings

    def decode(self, latent_embeddings):
        reconstructed_prose_vector = self.decoder(latent_embeddings)
        return reconstructed_prose_vector

    def loss(self, reconstructed, original_indices, loss_function):
        return loss_function(reconstructed.float(), self.og_embeddings[original_indices].float())
