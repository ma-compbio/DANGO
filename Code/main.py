...
def create_dango_list(ckpt_list , get_node_embedding=False):
    ...
    for checkpoint in ckpt_list:
        ...
        metaembedding_list.append(node_embedding)

    if get_node_embedding:
        return dango_list, metaembedding_list
...
if __name__ == '__main__':
    ...
    if 'eval-debug' in args.mode:
        ...
        if args.modelfolder is not None and os.path.exists("../Temp/%s" % args.modelfolder):
            ckpt_path = "../Temp/%s/Experiment_model_list" % args.modelfolder
            ckpt_list = list(torch.load(ckpt_path, map_location='cpu'))
