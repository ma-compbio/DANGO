...
device = 'cpu' if os.environ["device"] == 'cpu' else int(os.environ["device"])
from Modules import *
...
def train_epoch(model, embed_nn_list, recon_nn_list, data, recon_adj_list, optimizer, batch_size, device, ppi_nn=None):
    if len(data) == 2:
        tuples, y = data
    ...
    for i in bar:
        ...
        if ppi_nn is not None:
            ppi_nn.to(device)
            original_indices = batch_recon_index.long()
            encodings = ppi_nn(original_indices.to(device))
            preds = ppi_nn.decode(encodings.to(device).float())
            mse_loss = nn.MSELoss()
            loss_ppi += ppi_nn.loss(preds, original_indices, mse_loss)
...
def eval_epoch(model, embed_nn_list, recon_nn_list, data, recon_adj_list, optimizer, batch_size, device):
    ...
    with torch.no_grad():
        ...
        for i in bar:
            ...
            pred, loss_regress = forward_batch_hyper(model, batch_data, y=batch_y,
                                                     sample_weight=sample_weight[i * batch_size:(i + 1) * batch_size])
...
def train(model, embed_nn_list, recon_nn_list, training_data, validation_data, test_data, recon_adj_list, optimizer,
          epochs, batch_size, device, ppi_nn=None):
    checkpoint_list = []
    checkpoint_score = []
    ensemble_num = 5
    ...
    for epoch_i in range(epochs):
        ...
        loss_regress, loss_recon, corr1, corr2, corr1_pos, corr2_pos, roc, aupr = train_epoch(
            model, embed_nn_list, recon_nn_list, training_data, recon_adj_list, optimizer, batch_size, device, ppi_nn)
...
def pre_train(embed_nn_list, recon_nn_list, recon_adj_list, optimizer, epochs, batch_size, device, ppi_nn=None):
    x = torch.arange(1, gene_num, dtype=torch.long, device=device)
    # ppi_embeddings = torch.from_numpy(np.load("../data/ppi_embeddings.npy")).to(device)
...
if __name__ == '__main__':
    try:
        ...
        # print(sys.argv)
        positive_thres, gene_num, embed_dim, split_loc, save_dir, save_string, withPPI = float(sys.argv[1]), int(
            sys.argv[2]), int(sys.argv[3]), sys.argv[4], sys.argv[5], sys.argv[6], sys.argv[7]
        ...
        ckpt_list, score_list, hypersagnn, graphsage_embedding, recon_nn, optimizer, ensemble_part = one_training_procedure(
            (train_data, train_y),
            (valid_data, valid_y),
            (test_data, test_y), gene_num, withPPI)
